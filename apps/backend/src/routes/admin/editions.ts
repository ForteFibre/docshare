...
const ruleUpdateSchema = z.object({
  ruleDocuments: z.array(
    z.object({ label: z.string(), s3_key: z.string().min(1), mime_type: z.string().min(1) }),
  ),
});
...
const editionSchema = z.object({
  ...
  ruleDocuments: z
    .array(z.object({ label: z.string(), s3_key: z.string(), mime_type: z.string() }))
    .nullable(),
  sharingStatus: z.enum(['draft', 'accepting', 'sharing', 'closed']),
...
export const adminEditionRoutes = new OpenAPIHono<{ Variables: AppVariables }>();
